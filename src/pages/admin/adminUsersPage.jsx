...
export default function AdminUsersPage() {
    ...
    useEffect(() => {
        document.title = "Manage Users | IONIX Computers";
        fetchUsers();
    }, []);
            ...
            <div className="sticky top-0 z-10 w-full min-h-[90px] rounded-2xl bg-gradient-to-r from-accent to-blue-600 text-white border border-white/10 shadow-lg flex lg:flex-row flex-col lg:items-center justify-between px-6 py-4 mb-6 gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-white/80 mt-0.5">Manage registration roles and access controls</p>
                    </div>
                </div>
...

            {
                isLoaded ?
                    <div className="w-full overflow-x-auto custom-scrollbar-light rounded-2xl border border-gray-200 bg-white shadow-sm">
                        <table className="w-full min-w-[1000px] text-sm text-gray-700 whitespace-nowrap">
                            <thead className="bg-gray-50 text-gray-500 text-[11px] font-bold uppercase tracking-wider border-b border-gray-200">
                                <tr>
                                    ...
                                    <th className="text-left px-5 py-4">Email Verification</th>
                                    ...
                                    filteredUsers.map((item) => {
                                        return (
                                            ...
                                            >
                                                <td className="px-5 py-4">
                                                    <div className="flex items-center gap-3">
                                                        <img
                                                            ...
                                                            className="w-10 h-10 object-cover rounded-full border border-gray-200 bg-gray-100 shadow-sm"
                                                            onError={(e) => {
                                                ...
                                                <td className="px-5 py-4">
                                                    <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold ${item.isAdmin
                                                        ? "bg-purple-50 text-purple-600 border border-purple-100"
                                                        : "bg-blue-50 text-blue-600 border border-blue-100"
                                                    }`}>
                                                        <span className={`w-1.5 h-1.5 rounded-full ${item.isAdmin ? "bg-purple-500" : "bg-blue-500"}`} />
                                                        {item.isAdmin ? "Admin" : "Customer"}
                                                    </span>
                                                </td>

                                                <td className="px-5 py-4">
                                                    <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold ${!item.isBlocked
                                                        ? "bg-green-50 text-green-600 border border-green-100"
                                                        : "bg-red-50 text-red-600 border border-red-100"
                                                    }`}>
                                                        <span className={`w-1.5 h-1.5 rounded-full ${!item.isBlocked ? "bg-green-500 animate-pulse" : "bg-red-500"}`} />
                                                        {item.isBlocked ? "Blocked" : "Active"}
                                                    </span>
                                                </td>

                                                <td className="px-5 py-4">
                                                    <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold ${item.isEmailVerified
                                                        ? "bg-teal-50 text-teal-600 border border-teal-100"
                                                        : "bg-amber-50 text-amber-600 border border-amber-100"
                                                    }`}>
                                                        <span className={`w-1.5 h-1.5 rounded-full ${item.isEmailVerified ? "bg-teal-500" : "bg-amber-500 animate-pulse"}`} />
